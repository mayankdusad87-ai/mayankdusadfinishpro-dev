        ...
        <div className="p-4 md:p-6">
          ...
          {pendingWork.length === 0 ? (
            ...
            <div className="rounded-xl border border-gray-200 overflow-hidden">
              {pendingWork.map((stageGroup) => {
                const isCollapsed = collapsedStages.has(stageGroup.stage);
                return (
                  <div key={stageGroup.stage}>
                    {/* Stage header */}
                    <button
                      onClick={() => toggleStageCollapse(stageGroup.stage)}
                      className="w-full flex items-center justify-between px-4 py-2.5 bg-[#162032] cursor-pointer"
                    >
                      <div className="flex items-center gap-2">
                        <svg className={`w-3.5 h-3.5 text-[#C8922A] transition-transform ${isCollapsed ? '' : 'rotate-90'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
                        </svg>
                        <span className="text-white text-xs font-semibold">{stageGroup.stage}</span>
                      </div>
                      <span className="text-gray-400 text-[11px]">{stageGroup.subStages.length} sub-stage{stageGroup.subStages.length !== 1 ? 's' : ''} pending</span>
                    </button>

                    {/* Sub-stage rows */}
                    {!isCollapsed && stageGroup.subStages.map((sub, idx) => {
          ...
          {pendingWork.length > 0 && (
            <div className="text-[11px] text-gray-400 mt-2.5">
              Completed sub-stages are hidden. Only floors with pending work are shown.
            </div>
          )}
        </div>
      </div>
