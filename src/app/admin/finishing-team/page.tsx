...
import type { FinishingTeamUser } from '@/lib/supabase-data';
import { useDataLoader } from '@/hooks/use-data-loader';
import Modal from '@/components/shared/Modal';

export default function FinishingTeamPage() {
  const { data: users, loading, refresh: loadUsers } = useDataLoader(
    () => getFinishingTeamUsers(),
    [] as FinishingTeamUser[],
  );
  ...
  const [resetTarget, setResetTarget] = useState<FinishingTeamUser | null>(null);
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    const result = await createFinishingTeamUser(email, password, fullName, phone || undefined);
    if (result.error) {
  ...
  async function handleToggle(user: FinishingTeamUser) {
    const action = user.is_active ? 'deactivate' : 'reactivate';
    if (!confirm(`Are you sure you want to ${action} ${user.full_name}?`)) return;
    const result = await toggleFinishingTeamUserStatus(user.id, !user.is_active);
    if (result.error) {
      setError(result.error);
    } else {
      await loadUsers();
  ...
  function openResetPassword(user: FinishingTeamUser) {
    setResetTarget(user);
    setNewPassword('');
    setResetError('');
    setResetSuccess('');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Finishing Team</h1>
          <p className="mt-1 text-sm text-gray-500">
            Create and manage finishing team accounts.
          </p>
        </div>
        <button
              ...
              {users.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-5 py-10 text-center text-gray-400">
                    No finishing team users yet. Click &ldquo;Add Finishing Team User&rdquo; to create one.
                  </td>
                </tr>
              )}
              {users.map((user) => (
      ...
      {/* Create Form Modal */}
      <Modal open={showForm} onClose={() => setShowForm(false)} title="Add Finishing Team User">
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                type="submit"
                disabled={saving}
                className="px-4 py-2 text-sm font-medium text-white bg-primary rounded-lg hover:bg-primary-dark disabled:opacity-50 flex items-center gap-2 cursor-pointer"
              >
